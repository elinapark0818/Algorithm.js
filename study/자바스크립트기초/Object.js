// 함수에서 객체에 키 값을 할당할 때,
// 인자로 받아오는 값이 변수로 선언된 내용이기 때문에,
// 변수에 키값을 할당하기 위해서는 obj[변수명] = value 으로 키값을 할당한다
// 객체의 반복문은 for in
// 배열의 반복문은 for of
...
function mostFrequentCharacter(str) {
  let obj = { maxCount: 0, frequentChar: "" };
  for (let i = 0; i < str.length; i++) {
    if (str[i] === " ") {
      continue;
    }
    ...
    if (obj[str[i]] > obj["maxCount"]) {
      obj["maxCount"] = obj[str[i]];
      obj["frequentChar"] = str[i];
    }
  }
  return obj["frequentChar"];
}
...
// dot notation 으로 객체를 할당하면, 객체 타입이 string이다
let kimcoding = {};
let key = "isMale";
let value = true;
